...
- 本轮 release candidate 的最终客户端基线会在配套二进制安装后再次运行 `SC.Diagnostics.RunPerformanceBaseline()` 与 `.solocollections benchmark`；结果通过 `SC_PERF` 和 `module.solocollections.performance` 结构化行留存。
