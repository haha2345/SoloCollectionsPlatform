...
class StableIdTests(unittest.TestCase):
    ...
    def test_unreserved_collection_is_rejected_then_changes_hash_when_reserved(self):
        csv_path = self.source / "collections" / "mounts.csv"
        with csv_path.open("a", encoding="utf-8", newline="") as handle:
            handle.write("mount,1000,test_mount,0,active,Test Mount,测试坐骑,unrestricted,3.3.5.12340,spell,123,spell,123,true,mount.test,TEST_MOUNT\n")
        ...
        ids["reservations"]["collections"].append(
            {"id": 1000, "key": "test_mount", "ordinal": 0, "lifecycle": "active"}
        )
        self.save("catalog/ids.json", ids)
        ...
        self.assertEqual("test_mount", populated["collections"][0]["collectionKey"])
